...
export default function TodoView(){
    ...
    useEffect(()=>{
        fetch(`https://todo-list-server-c69d117e8112.herokuapp.com/api/get`)
        .then((res)=>res.json())
    ...
    const del = async(id) => {
         try{
            const delRes = await axios.delete(`https://todo-list-server-c69d117e8112.herokuapp.com/api/del/${id}`)
    ...
    const updateTask = async(item) => {
        ...
        if(update!==null){
            try{
                const res = await axios.patch(`https://todo-list-server-c69d117e8112.herokuapp.com/api/completed/${update.id}`,update);
